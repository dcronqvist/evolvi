    ...
    public class Graph
    {
        ...
        public void Draw(List<float> values, Color color)
        {
            ...
            for (int yy = 0; yy < 10; yy++)
            {
                ...
                {
                    GameHelper.DrawLine(new Vector2(Position.X, Position.Y - (yy * (Size.Y / 10))), new Vector2(Position.X + Size.X, Position.Y - (yy * (Size.Y / 10))), Color.White, 1, 0f);
                    //GameHelper.DrawLine(new Vector2(Position.X + (xx * (Size.X / 10)), Position.Y), new Vector2(Position.X + (xx * (Size.X / 10)), Position.Y - Size.Y), Color.DimGray, 1, 0f);
                }
            }
            ...
            if (this.Type == GraphType.Line)
            {
                ...
                for (int i = values.Count - 1; i < values.Count; i++)
                {
                    string val = (Math.Round(values[i] * 100) / 100).ToString();

                    GameHelper.SpriteBatch.DrawString(GameHelper.Font, val, new Vector2(i * xScale + Position.X, Position.Y - ((values[i] / MaxValue) * Size.Y)), Color.Black);
                }
